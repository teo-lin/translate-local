...
> **Repo:** `translate-local` (lean variant). Everything below the "Repo-specific
> notes" section is shared verbatim with the `translate-renpy` test suite
> (`tests/TESTS-README.md` there) - keep the two in sync.

## Repo-specific notes (translate-local)

- This repo ships no bundled `games/Example/`, so `test_e2e_example.py` skips
  unless you supply game files at the path it expects.
- `test_unit_renpy_tags.py` covers the Ren'Py tag extractor/restorer used by
  `extract.py` / `merge.py` (this repo still uses the Ren'Py `.rpy` format; only
  the SDK/tools download is omitted).
