...
# Determine poly_trans package root
# This file is at: src/poly_trans/prompts.py
# We want: src/poly_trans/
PACKAGE_ROOT = Path(__file__).parent

# Load prompt templates once at module import
# Fallback hierarchy: translate_uncensored.txt → translate.txt
try:
    TRANSLATION_PROMPT_TEMPLATE = (PACKAGE_ROOT / "data" / "prompts" / "translate_uncensored.txt").read_text(encoding='utf-8')
except FileNotFoundError:
    try:
        TRANSLATION_PROMPT_TEMPLATE = (PACKAGE_ROOT / "data" / "prompts" / "translate.txt").read_text(encoding='utf-8')
    except FileNotFoundError:
        raise FileNotFoundError(
            f"Translation prompt template not found. Please create one of:\n"
            f"  - {PACKAGE_ROOT / 'data' / 'prompts' / 'translate_uncensored.txt'}\n"
            f"  - {PACKAGE_ROOT / 'data' / 'prompts' / 'translate.txt'}"
        )

# Fallback hierarchy: correct_uncensored.txt → correct.txt
try:
    CORRECTION_PROMPT_TEMPLATE = (PACKAGE_ROOT / "data" / "prompts" / "correct_uncensored.txt").read_text(encoding='utf-8')
except FileNotFoundError:
    try:
        CORRECTION_PROMPT_TEMPLATE = (PACKAGE_ROOT / "data" / "prompts" / "correct.txt").read_text(encoding='utf-8')
    except FileNotFoundError:
        raise FileNotFoundError(
            f"Correction prompt template not found. Please create one of:\n"
            f"  - {PACKAGE_ROOT / 'data' / 'prompts' / 'correct_uncensored.txt'}\n"
            f"  - {PACKAGE_ROOT / 'data' / 'prompts' / 'correct.txt'}"
        )
