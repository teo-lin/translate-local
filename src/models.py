"""
Data models for the generic YAML translation system.

This module defines TypedDict classes and enums for type-safe data handling
across the translation workflow. Supports both legacy Renpy models (for backwards
compatibility) and new generic translation models.
"""
...
class TagFormatType(str, Enum):
    """Tag/markup format types."""
    HTML = "html"
    MARKDOWN = "markdown"
    CUSTOM_BRACES = "custom_braces"
    CUSTOM_BRACKETS = "custom_brackets"
    AUTO = "auto"


class BlockType(str, Enum):
    """Types of blocks in a Ren'Py translation file (LEGACY)."""
...
class FileStructureType(str, Enum):
    """Overall structure type of a translation file (LEGACY)."""
...
class CharacterType(str, Enum):
    """Character types for classification (LEGACY)."""
    MAIN = "main"
...
# ============================================================================
# TYPED DICTIONARIES - GENERIC TRANSLATION MODELS
# ============================================================================

class GenericTagInfo(TypedDict):
    """Information about a tag in generic text."""
    pos: int                        # Position in clean text
    content: str                    # Tag content (e.g., "<b>", "**", "{name}")
    type: str                       # Tag type identifier


class FileMetadata(TypedDict, total=False):
    """Metadata for a generic translation file."""
    source_language: str            # Source language code (e.g., "en")
    target_languages: List[str]     # Target language codes (e.g., ["ro", "es"])
    tag_format: str                 # Tag format: "html", "markdown", "auto", etc.
    created_at: str                 # ISO timestamp
    updated_at: str                 # ISO timestamp


class TranslationBlock(TypedDict, total=False):
    """
    A single translation block in the generic YAML format.

    Each block contains source text and translations in multiple languages.
    """
    source_text: str                # Original text (with tags)
    translations: Dict[str, str]    # {lang_code: translation}
    context: Optional[str]          # Context information
    category: Optional[str]         # Category: "dialogue", "ui", "menu", etc.
    speaker: Optional[str]          # Speaker name for dialogue
    tags: Optional[List[GenericTagInfo]]  # Tag information for restoration


class TranslationFile(TypedDict):
    """Complete generic translation file structure."""
    metadata: FileMetadata
    blocks: Dict[str, TranslationBlock]  # block_id -> TranslationBlock


# ============================================================================
# TYPED DICTIONARIES - LEGACY RENPY MODELS
# ============================================================================
